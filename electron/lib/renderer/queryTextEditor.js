...
const createQueryTextEditor = (notebookEl) => {
  ...
  if (!editor) {
    ...
    lineNumbers.className = "whitespace-pre text-white/40 select-none text-xl w-8 flex-shrink-0";
    lineNumbers.textContent = "1";
    inputRow.appendChild(lineNumbers);

    const textarea = document.createElement("textarea");
    textarea.className = "w-full bg-transparent border-none text-green-500 focus:outline-none text-xl ml-1 resize-none overflow-hidden";
