...
ipcMain.handle('get-mssql-presets', () => {
  ...
  return (appPresets.db_presets || [])
    .filter(p => p.db_type === 'mssql')
    .map(p => p.name);
});

// IPC handler for dynamic database queries.
ipcMain.handle('database-query', async (event, queryOptions) => {
  // queryOptions is expected to be an object: { query, preset }
  console.log('IPC handler "database-query" invoked with options:', queryOptions);
  const { query, preset } = queryOptions;
  ...
  if (preset) {
    ...
    // If no preset is specified, use the first MSSQL preset as default if available.
    if (defaultPreset) {
      configToUse = {
  ...
  try {
    ...
    // Close the connection (for a production app, consider using a connection pool).
    sql.close();
