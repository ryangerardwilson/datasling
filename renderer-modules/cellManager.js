  ...
  if (!inputCell) {
    ...
    textarea.addEventListener('input', () => {
      ...
      if (currentLine === '@preset::') {
        ipcRenderer.invoke('get-mssql-presets').then(presets => {
          showPresetDropdown(presets, (selected) => {
    ...
    textarea.addEventListener('keydown', async (e) => {
      if (e.key === 'Enter' && e.ctrlKey) {
        ...
        // If semicolons are present, split accordingly; otherwise, perform newline-based segmentation.
        if (queryText.indexOf(';') !== -1) {
          const queries = queryText.split(';').map(q => q.trim()).filter(q => q.length);
          for (const query of queries) {


            if (query.startsWith('@preset::')) {
              currentPreset = query.replace('@preset::', '').trim();
          ...
          for (const line of lines) {
            ...
            if (trimmedLine.startsWith('@preset::')) {
              // If there's an accumulated query segment, execute it.
              if (segment.trim() !== "") {
              ...
              currentPreset = trimmedLine.replace('@preset::', '').trim();
              console.log(`Preset updated to "${currentPreset}"`);
            } else {
              segment += " " + line;
            }
    ...
    if (existingTextarea) {
      setTimeout(() => { existingTextarea.focus(); }, 50);
    }
  }
  return inputCell;
}
